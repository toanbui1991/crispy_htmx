...
def index(request):
    if request.method == 'GET':
        ...
        return render(request, 'crispy_htmx/index.html', context)
        ...
        if form.is_valid():
            user = form.save()
            login(request, user)
            template = render(request, 'crispy_htmx/profile.html')
...
def check_username(request):
    ...
    return render(request, 'crispy_htmx/partials/field.html', context)
